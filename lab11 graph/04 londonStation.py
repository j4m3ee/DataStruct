...
Map = {"Acton Town" : ["Ealing Common","South Ealing","Turnham Green","Chiswick Park"],
...
class graph():
    def __init__(self) :
        self.weight = dict()
        self.minimumWeight = 9999999

    def addPath(self,start,path):
        self.weight[start] = self.weight.get(start,[]) + [path]
    
    def minPath(self,src,dest,visited):
        minimum = self.minimumWeight

        for v in self.weight:
            if dest[v][1] < minimum and v not in visited:
                minimum = dest[v][1]
                src = v
    ...
    def dijkstra(self,src):
        ...
        for v in self.weight:
            dist[v] = [v,self.minimumWeight]
        dist[src] = [src,0]

        for v in self.weight:
            src = self.minPath(src,dist,visited)
            visited += [src]
...
if __name__ == "__main__":
    ...
    print('From: {}'.format(inp[0]))
    print('To: {}'.format(inp[1]))
    print('Searching shortest route... This may take a while...\nSuggested Route: ')
    print(g.shortestPath(inp[0],inp[1]))